...
const MODERN_SIGNATURE_FUNCTION_REGEXP = "([a-zA-Z$0-9_]+)\\s*=\\s*function\\s*\\([^)]*\\)\\s*\\{[^}]*?split\\s*\\(\\s*[\"'']?\\s*[\"'']?\\s*\\)[^}]*?join\\s*\\(\\s*[\"'']?\\s*[\"'']?\\s*\\)[^}]*?\\}";

const JDD_FUNCTION_REGEXP = "([a-zA-Z$0-9_]+)\\s*=\\s*function\\s*\\([^)]*\\)\\s*\\{[^}]*?\\[[^\\]]+\\]\\s*\\([^}]*?\\)[^}]*?return[^}]*?\\[[^\\]]+\\]\\s*\\([^}]*?\\)[^}]*?\\}";

const V_ARRAY_REGEXP = "var\\s+([a-zA-Z$0-9_]+)\\s*=\\s*[\"']([^\"']+)[\"']\\s*\\.split\\s*\\(\\s*[\"'][^\"']*[\"']\\s*\\)";

const ZK_OBJECT_REGEXP = "var\\s+([a-zA-Z$0-9_]+)\\s*=\\s*\\{[^}]+\\};";
...
const extractModernSignature = (body) => {
  try {
    const vArrayMatch = body.match(new RegExp(V_ARRAY_REGEXP, 'm'));
    if (!vArrayMatch) return null;
    
    const vArrayName = vArrayMatch[1];
    const vArrayValues = vArrayMatch[2].split(';');
    
    console.log(`Found V array: ${vArrayName} with ${vArrayValues.length} elements`);
    
    const zkObjectMatch = body.match(new RegExp(ZK_OBJECT_REGEXP, 's'));
    if (!zkObjectMatch) return null;
    
    const zkObjectName = zkObjectMatch[1];
    console.log(`Found Zk object: ${zkObjectName}`);
    
    const jddPattern = `([a-zA-Z$0-9_]+)\\s*=\\s*function\\s*\\([^)]*\\)\\s*\\{[^}]*?${vArrayName}\\[[^}]*?${zkObjectName}\\[[^}]*?return[^}]*?\\}`;
    const jddMatch = body.match(new RegExp(jddPattern, 's'));
    
    if (!jddMatch) return null;
    
    const jddFunctionName = jddMatch[1];
    const jddFunctionBody = jddMatch[0];
    
    console.log(`Found jDD-style function: ${jddFunctionName}`);
    
    const sigCallPattern = new RegExp(`${jddFunctionName}\\s*\\([^)]*\\.s[^)]*\\)`, 'g');
    const sigCalls = [...body.matchAll(sigCallPattern)];
    
    if (sigCalls.length === 0) {
      console.log(`Function ${jddFunctionName} not called with .s parameter, skipping`);
      return null;
    }
    
    console.log(`Function ${jddFunctionName} called ${sigCalls.length} times with .s parameter`);
    
    return {
      vArray: vArrayMatch[0],
      zkObject: zkObjectMatch[0],
      signatureFunction: jddFunctionBody,
      functionName: jddFunctionName
    };
    
  } catch (e) {
    console.error("Error in extractModernSignature:", e);
    return null;
  }
}
...
const extractDecipherFunc = (body, name, code) => {
  try {
    ...
    if (helperMatch) {
      ...
      const hasReverseKey = extractDollarEscapedFirstGroup(REVERSE_PATTERN, actionBody);
      const hasSliceKey = extractDollarEscapedFirstGroup(SLICE_PATTERN, actionBody);
      const hasSpliceKey = extractDollarEscapedFirstGroup(SPLICE_PATTERN, actionBody);
      const hasSwapKey = extractDollarEscapedFirstGroup(SWAP_PATTERN, actionBody);

      if (hasReverseKey || hasSliceKey || hasSpliceKey || hasSwapKey) {
    ...
    console.log("Traditional extraction failed, trying modern signature extraction...");
    const modernSig = extractModernSignature(body);
    if (modernSig) {
      console.log("Modern signature extraction successful, creating wrapper function");
      
      const wrapperFunction = `
        ${modernSig.vArray}
        ${modernSig.zkObject}
        ${modernSig.signatureFunction}
        
        var ${DECIPHER_FUNC_NAME} = function(${DECIPHER_ARGUMENT}) {
          return ${modernSig.functionName}(${DECIPHER_ARGUMENT});
        };
      `;
  ...
  if (!decipherFunc && !decipherWarning) {
    console.warn(
      ...
      `Please report this issue by uploading the "${utils.saveDebugFile(
        "player-script.js",
        body,
      )}" file on https://github.com/tieubao9k/ytdl-core/issues.`
    );
...
exports.decipherFormats = async (formats, html5player, options) => {
  try {
    ...
    formats.forEach(format => {
      exports.setDownloadURL(format, decipherScript, nTransformScript);
      const key = format.url || `itag_${format.itag}`;
      decipheredFormats[key] = format;
    });
