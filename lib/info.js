...
// FastInfoRetriever integrated below
// DisTube patterns integrated directly into sig.js
...
// Fast info retriever instance
let fastInfoRetriever = null;
const getFastInfoRetriever = (options = {}) => {
  if (!fastInfoRetriever || options.agent) {
    // Create new instance if agent is provided or first time
    fastInfoRetriever = new FastInfoRetriever(options.agent);
  }
  return fastInfoRetriever;
};
...
exports.getBasicInfo = async(id, options) => {
  // Try fast Android client first if fast mode enabled and not disabled
  if (options && options.fastMode && options.fastMode !== 'disabled') {
    try {
      const fastRetriever = getFastInfoRetriever(options);
      const fastInfo = await fastRetriever.getFastVideoInfo(id);
      
      if (fastInfo && fastInfo.videoDetails && fastInfo.streamingData) {
        // Convert fast info to ytdl format
        const formats = parseFormats(fastInfo);
        const info = {
          player_response: fastInfo,
          formats: formats,
          related_videos: [],
        };
        
        // Add additional properties similar to standard method
        Object.assign(info, {
          related_videos: [], // Fast mode doesn't get related videos
        });

        const media = {}; // Fast mode simplified media
        const additional = {
          author: extras.getAuthor(info) || { name: fastInfo.videoDetails.author || 'Unknown' },
          media,
          likes: 0,
          dislikes: 0,
        ...
        info.videoDetails = extras.cleanVideoDetails(Object.assign({},
          fastInfo.videoDetails, additional), info);
        
        return info;
      }
    } catch (error) {
      // Fallback to standard method silently
      // Continue to standard method below
    }
  }

  // Standard ytdl-core method
...
const parseFormats = player_response => {
  ...
  // Enhanced format processing to add missing fields
  formats.forEach(format => {
    // Extract container from mimeType
    if (format.mimeType && !format.container) {
      const mimeMatch = format.mimeType.match(/^(\w+)\/(\w+)/);
      if (mimeMatch) {
        format.container = mimeMatch[2] === 'mp4' ? 'mp4' : 
                          mimeMatch[2] === 'webm' ? 'webm' : 
                          mimeMatch[2] === '3gpp' ? '3gp' : mimeMatch[2];
      }
    }
    
    // Determine hasVideo/hasAudio based on format properties
    format.hasVideo = !!(format.width || format.height || 
      (format.mimeType && format.mimeType.startsWith('video/')));
      
    format.hasAudio = !!(format.audioSampleRate || format.audioBitrate ||
      format.audioChannels || (format.mimeType && format.mimeType.startsWith('audio/')));
    
    // Set audioBitrate from averageBitrate if missing
    if (!format.audioBitrate && format.averageBitrate && format.hasAudio && !format.hasVideo) {
      format.audioBitrate = Math.round(format.averageBitrate / 1000);
    }
    
    // Extract codecs info if available  
    if (format.mimeType && format.mimeType.includes('codecs=')) {
      const codecMatch = format.mimeType.match(/codecs="([^"]+)"/);
      if (codecMatch) {
        format.codecs = codecMatch[1];
      }
    }
  });
...
exports.getInfo = async(id, options) => {
  ...
  if (options && options.fastMode) {
    try {
      ...
      // If using fast mode and we have direct URLs, skip signature decryption
      if (info.formats.length > 0) {
        const hasDirectUrls = info.formats.some(format => format.url);
        if (hasDirectUrls) {
          // Mark as full info and return (skip HTML5player extraction)
          info.full = true;
          return info;
        }
      }
    } catch (error) {
      // Fast mode failed, use standard method
      // Fast mode failed, fallback silently
    }
  }
...
/**
 * Fast info retrieval using Android client (Integrated from fast-info.js)
 */
class FastInfoRetriever {
  constructor(customAgent = null) {
    // Updated User-Agent and headers to match latest working patterns
    this.userAgent = 'com.google.android.youtube/19.14.43 (Linux; U; Android 11) gzip';
    this.apiKey = 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8';
    this.visitorData = null;
    this.clientVersion = '19.14.43';
    this.clientName = 'ANDROID_MUSIC';
    
    // Use custom agent if provided, otherwise default connection pooling
    if (customAgent) {
      this.customAgent = customAgent;
      this.httpsAgent = customAgent.agent || customAgent.dispatcher;
    } else {
  ...
  /**
   * Optimized HTTP request with compression and agent support
   */
  async makeRequest(url, options = {}) {
    // Use miniget for better agent integration if custom agent is provided
    if (this.customAgent) {
      ...
      // Add agent support
      if (this.customAgent.dispatcher) {
        requestOptions.dispatcher = this.customAgent.dispatcher;
      } else if (this.customAgent.agent) {
        requestOptions.agent = this.customAgent.agent;
      }
      
      return miniget(url, requestOptions).text();
    }
    
    // Fallback to native HTTP for default agent
    const https = require('https');
  ...
  async getFastVideoInfo(videoId) {
    ...
    // Use WEB client as fallback - more reliable
    const payload = {
      context: {
        client: {
          clientName: 'WEB',
...
// Export FastInfoRetriever for optional use
exports.FastInfoRetriever = FastInfoRetriever;
