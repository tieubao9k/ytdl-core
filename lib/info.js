...
const distubeFallback = require('./distube-fallback');
...
exports.getBasicInfo = async(id, options) => {
  ...
  if (options && options.fastMode) {
    try {
      ...
      // Fallback to DisTube if available, then standard method
      console.warn('Fast mode failed:', error.message);
      
      if (distubeFallback.isAvailable()) {
        try {
          console.log('🔄 Trying DisTube fallback...');
          return await distubeFallback.getBasicInfo(id, options);
        } catch (distubeError) {
          console.warn('DisTube fallback also failed:', distubeError.message);
        }
      }
      
      console.log('🐢 Using standard ytdl-core method');
    }
  }
...
exports.getInfo = async(id, options) => {
  ...
  if (options && options.fastMode) {
    try {
      ...
      // Fast mode failed, try DisTube fallback
      if (distubeFallback.isAvailable()) {
        try {
          console.log('🔄 Fast getInfo failed, trying DisTube fallback...');
          return await distubeFallback.getInfo(id, options);
        } catch (distubeError) {
          console.warn('DisTube getInfo fallback also failed:', distubeError.message);
        }
      }
    }
  }
