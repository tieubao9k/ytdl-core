 ...
 * NOTE: With ANDROID InnerTube client, this now returns full info (same as getInfo)
...
exports.getBasicInfo = async (id, options = {}) => {
  // With InnerTube API, basic info and full info are the same
  // Just redirect to full getInfo
  return exports(id, options);
};
...
const retryFunc = async (func, args, options) => {
  ...
  while (currentTry <= options.maxRetries) {
    try {
      ...
      if (err?.statusCode < 500 || currentTry >= options.maxRetries) {
        throw err;
      } else {
...
/**
 * Gets info from a video using Android InnerTube client.
 * Handles signature decryption automatically when needed.
 *
 * @param {string} id
 * @param {Object} options
 * @returns {Promise<Object>}
...
exports.getInfo = async (id, options) => {
  ...
  // Use ANDROID client (only client that works reliably with all videos)
  const innerTubeInfo = await innerTube.getInfo(id, {
    headers: options.requestOptions?.headers
  });

  if (!innerTubeInfo.success) {
