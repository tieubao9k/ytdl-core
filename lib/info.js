...
const innerTube = require("./innertube-clients");
...
const getHTML5player = body => {
  // Extract actual player URL from YouTube response (dynamic player ID)
...
const retryFunc = async (func, args, options) => {
  ...
  while (currentTry <= options.maxRetries) {
    try {
      ...
      const detection = antiBotManager.handleBotDetection(err);
      
      if (detection.detected) {
        console.warn('🤖 YouTube bot detection triggered, applying anti-bot measures...');
        // Force new session and headers for next retry
        const wait = Math.min(++currentTry * options.backoff.inc * 2, options.backoff.max); // Double delay for bot detection
        await new Promise(resolve => setTimeout(resolve, wait));
      } else if (err?.statusCode < 500 || currentTry >= options.maxRetries) {
        throw err;
      } else {
...
const getWatchHTMLPage = async (id, options) => {
  ...
  try {
    try {
      // Updated patterns for YouTube 2025
      info.player_response =
        ...
        utils.tryParseBetween(body, '"initialPlayerResponse":', ',"secondaryResults"') ||
        utils.tryParseBetween(body, '"initialPlayerResponse":', ',"playerResponse"') ||
        utils.tryParseBetween(body, "ytInitialPlayerResponse\":", "};") ||
        utils.tryParseBetween(body, "ytInitialPlayerResponse\":", ";") ||
        utils.tryParseBetween(body, 'ytInitialPlayerResponse":', '}}}') ||
        // Embedded patterns
        utils.tryParseBetween(body, 'ytcfg.set({"EXPERIMENT_FLAGS"', '"VISITOR_DATA"') ||
        // Advanced pattern matching for 2025
      ...
      // Fallback with advanced extraction
      info.player_response = 
        utils.extractYouTubeJSON(body, 'ytInitialPlayerResponse') ||
        utils.extractYouTubeJSON(body, 'initialPlayerResponse') ||
        findPlayerResponse("watch.html", findJSON("watch.html", "player_response", body, /\bytplayer\.config\s*=\s*{/, "</script>", "{"));
    }
    ...
    info.response =
      ...
      // New patterns for 2025
      utils.tryParseBetween(body, '"initialData":', ',"visitorData"') ||
      utils.tryParseBetween(body, '"initialData":', ',"trackingParams"') ||
      utils.tryParseBetween(body, "ytInitialData\":", "};") ||
      utils.tryParseBetween(body, "ytInitialData\":", ";") ||
      utils.tryParseBetween(body, 'ytInitialData":', '}}}') ||
      // Advanced pattern matching for 2025
...
const estimateAudioBitrate = (format) => {
  // Return existing audio bitrate if available
  if (format.audioBitrate) return format.audioBitrate;
  
  // Estimate based on codec and quality
  if (format.audioCodec) {
    const codec = format.audioCodec.toLowerCase();
    
    // High-quality codecs
    if (codec.includes('opus')) {
      return format.quality === 'medium' ? 160 : 128;
    }
    if (codec.includes('aac') || codec.includes('mp4a')) {
      return format.quality === 'medium' ? 128 : 96;
    }
    if (codec.includes('vorbis')) {
      return format.quality === 'medium' ? 192 : 128;
    }
  ...
  // Fallback estimation based on container and quality
...
exports.getInfo = async (id, options) => {
  ...
  // Try InnerTube multi-client approach first (Lavalink-style)
  // This provides direct URLs without signature decipher for ANDROID/IOS clients
  if (options.useInnerTube !== false) {
    try {
      const innerTubeInfo = await innerTube.getInfo(id, {
        clients: options.innerTubeClients || ['ANDROID', 'ANDROID_VR', 'IOS', 'WEB'],
        headers: options.requestOptions?.headers
      });

      if (innerTubeInfo.success && innerTubeInfo._innerTube.directUrls > 0) {
        // Successfully got direct URLs from InnerTube
        return innerTubeInfo;
      }
    } catch (innerTubeError) {
      // InnerTube failed, fall back to traditional scraping
      console.warn('InnerTube approach failed, falling back to traditional method:', innerTubeError.message);
    }
  }
  ...
  try {
    ...
    if (options.playerClients.includes("WEB")) {
      let bestPlayerResponse = info.player_response;
  ...
  // Filter formats but be more lenient - keep formats with URLs even if missing other metadata
  info.formats = info.formats.filter(format => {
    return format && 
           format.url && 
           (format.mimeType || format.itag); // Allow formats with either mimeType or itag
  });
