...
const getFastInfoRetriever = () => {
  if (!fastInfoRetriever) {
    fastInfoRetriever = new FastInfoRetriever();
  }
...
exports.getBasicInfo = async(id, options) => {
  // Try fast Android client first if fast mode enabled
  if (options && options.fastMode) {
    try {
      const fastRetriever = getFastInfoRetriever();
      ...
      console.warn('Fast mode failed, using standard method:', error.message);
...
exports.getInfo = async(id, options) => {
  ...
  if (options && options.fastMode) {
    try {
      ...
      console.warn('Fast getInfo failed, using standard method:', error.message);
    }
  }
