...
const downloadFromInfoCallback = (stream, info, options) => {
  ...
  if (format.isHLS || format.isDashMPD) {
    ...
    if (shouldUseMultiThreadDownload) {
      // Enhanced multi-threaded chunked download
      ...
      let totalChunks = Math.ceil(contentLength / threadChunkSize);
      
      for (let i = 0; i < maxThreads && i * threadChunkSize < contentLength; i++) {
        ...
        threadReq.on('end', () => {
          ...
          if (completedChunks >= totalChunks || activeRequests.size === 0) {
            stream.end();
          }
        });
        ...
        threadReq.on('error', () => {
          ...
          // Fall back to single-thread on any thread error
