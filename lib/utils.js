...
/**
 * DisTube enhanced tryParseBetween for better JSON parsing
 */
exports.tryParseBetween = (body, left, right, prepend = "", append = "") => {
  try {
    let data = exports.between(body, left, right);
    if (!data) return null;
...
exports.checkForUpdates = () => {
  if (!process.env.YTDL_NO_UPDATE && !pkg.version.startsWith('0.0.0-') &&
    ...
    return miniget('https://api.github.com/repos/tieubao9k/ytdl-core/releases/latest', {
      headers: { 'User-Agent': 'ytdl-core' },
    }).text().then(response => {
      if (JSON.parse(response).tag_name !== `v${pkg.version}`) {
        // Enhanced ytdl-core with Android client - update check disabled for custom version
...
/**
 * DisTube debug file saving for troubleshooting
 */
exports.saveDebugFile = (filename, content) => {
  try {
    const path = `./debug-${filename}`;
