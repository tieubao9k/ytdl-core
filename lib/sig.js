...
exports.setDownloadURL = (format, decipherScript, nTransformScript) => {
  const decipher = url => {
    ...
    try {
      ...
      // Decipher failed, continue with next method
      return url;
    }
  };
  const ncode = url => {
    ...
    try {
      ...
      // N-code transform failed, continue
      return url;
    }
  };
...
exports.decipherFormats = async(formats, html5player, options) => {
  ...
  try {
    ...
    // Use DisTube fallback silently for better UX
    ...
    try {
      ...
      // DisTube signature extraction succeeded silently
    } catch (distubeError) {
      // DisTube signature extraction also failed
    }
  }
