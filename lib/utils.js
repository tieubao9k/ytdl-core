...
// Pool of modern User-Agents (Chrome 137-141)
const USER_AGENTS = [
...
// Pick one UA per process lifetime to stay consistent
const DEFAULT_USER_AGENT = USER_AGENTS[Math.floor(Math.random() * USER_AGENTS.length)];
...
exports.applyPlayerClients = options => {
  if (!options.playerClients || options.playerClients.length === 0) {
    options.playerClients = ["ANDROID_VR", "ANDROID", "IOS", "TV", "WEB_EMBEDDED"];
  }
};
