...
class SignatureDecoder {
  constructor() {
    ...
    this.remoteCipherUrl = null;
    this.remoteCipherPassword = null;
  }

  setRemoteCipher(url, password) {
    this.remoteCipherUrl = url;
    this.remoteCipherPassword = password;
  }
  ...
  async applyCipher(signature, cipher) {
    if (this.remoteCipherUrl) {
      try {
        const response = await request(`${this.remoteCipherUrl}/decipher`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': this.remoteCipherPassword || ''
          },
          body: JSON.stringify({
            signature,
            playerScriptUrl: cipher.scriptUrl
          })
        });

        if (response.statusCode === 200) {
          const result = await response.body.json();
          return result.deciphered || result.signature;
        }
      } catch (error) {
      }
    }
  ...
  async transformNParameter(nParam, cipher) {
    if (this.remoteCipherUrl) {
      try {
        const response = await request(`${this.remoteCipherUrl}/transform-n`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': this.remoteCipherPassword || ''
        ...
        if (response.statusCode === 200) {
          const result = await response.body.json();
          return result.transformed || result.nParam;
        }
      } catch (error) {
      }
    }
...
module.exports = {
  ...
  setRemoteCipher: (url, password) => decoderInstance.setRemoteCipher(url, password),
};
